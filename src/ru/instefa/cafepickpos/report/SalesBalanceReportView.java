...
public class SalesBalanceReportView extends JPanel {
	...
	private SimpleDateFormat fullDateFormatter = new SimpleDateFormat(DateUtil.getDatePattern("dd MMM yyyy, hh:mm a")); //$NON-NLS-1$
	...
	public SalesBalanceReportView() {
		...
		btnToday.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					PosLog.error(SalesBalanceReportView.class, ex.getMessage());
				}
			}
		});
		...
		topPanel.add(new JLabel(ru.instefa.cafepickpos.POSConstants.FROM + ":")); //$NON-NLS-1$ 
		topPanel.add(fromDatePicker);
		topPanel.add(new JLabel(ru.instefa.cafepickpos.POSConstants.TO + ":")); //$NON-NLS-1$ 
	...
	private void viewReport(Date fromDate, Date toDate) throws Exception {
		...
		map.put("userName", user == null ? ru.instefa.cafepickpos.POSConstants.ALL : user.getFullName()); //$NON-NLS-1$
