...
import ru.instefa.cafepickpos.model.dao.TerminalPrintersDAO;
...
import ru.instefa.cafepickpos.util.PrintServiceUtil;
...
public class ReceiptPrintService {
	...
	/*public static void printTicket(Ticket ticket) {
		try {
			...
			logger.error(ru.instefa.cafepickpos.POSConstants.PRINT_ERROR, e);
		}
	}*/
	...
	public static void printRefundTicket(Ticket ticket, RefundTransaction posTransaction) {
		try {
			TicketPrintProperties printProperties = new TicketPrintProperties(
				Messages.getString("ReceiptPrintService.120"),
				true, true, true);
			printProperties.setPrintCookingInstructions(false);
	...
	private static void populateRefundProperties(Set<PosTransaction> transactions, HashMap map) {
		if (transactions == null)
			return;
		TicketPrintProperties printProperties = new TicketPrintProperties(
			Messages.getString("ReceiptPrintService.120"), true, true, true); //$NON-NLS-1$
		printProperties.setPrintCookingInstructions(false);
	...
	private static String getCardInformation(PosTransaction transaction) {
		String string = "<br/>" + Messages.getString("ReceiptPrintService.121") + ": ------------------------";
		string += "<br/>" + Messages.getString("ReceiptPrintService.122") + ": " + transaction.getCardReader();
		string += "<br/> " + Messages.getString("ReceiptPrintService.123") + ": " + transaction.getCardType();
		try {
			String cardNumber = transaction.getCardNumber();
			if (transaction.getCardNumber() != null) {
				string += "<br/> " + Messages.getString("ReceiptPrintService.124") + ": **** **** **** " + cardNumber.substring(cardNumber.length() - 4, cardNumber.length());
			}
			if (transaction.getCardHolderName() != null) {
				string += "<br/> " + Messages.getString("ReceiptPrintService.125") + ": " + transaction.getCardHolderName();
			}
			if (transaction.getCardTransactionId() != null) {
				string += "<br/> " + Messages.getString("ReceiptPrintService.126") + ": " + transaction.getCardTransactionId();
			}
			string += "<br/> " + Messages.getString("ReceiptPrintService.127") + ": " + transaction.getCardAuthCode();
			if (transaction.getCardAID() != null) {
				string += "<br/> " + Messages.getString("ReceiptPrintService.128") + ": " + transaction.getCardAID();
			}
			if (transaction.getCardARQC() != null) {
				string += "<br/> " + Messages.getString("ReceiptPrintService.129") + ": " + transaction.getCardARQC();
			}
		} catch (Exception e) {
			PosLog.error(ReceiptPrintService.class, e.getMessage());;
		}
		return string;
	}
	...
	private static StringBuilder buildTicketHeader(Ticket ticket, TicketPrintProperties printProperties) {
		...
		if (driver != null) {
			beginRow(ticketHeaderBuilder);
			addColumn(ticketHeaderBuilder, Messages.getString("ReceiptPrintService.130")); //$NON-NLS-1$ //$NON-NLS-2$
			endRow(ticketHeaderBuilder);
		...
		if (orderType.isRequiredCustomerData()) {
			...
			if (StringUtils.isNotEmpty(customerName)) {
				...
				if (StringUtils.isNotEmpty(customerMobile)) {
					beginRow(ticketHeaderBuilder);
					addColumn(ticketHeaderBuilder, Messages.getString("ReceiptPrintService.131") + ": " + customerMobile); //$NON-NLS-1$
					endRow(ticketHeaderBuilder);
				}

				if (ticket.getDeliveryDate() != null) {
					beginRow(ticketHeaderBuilder);
					addColumn(ticketHeaderBuilder, Messages.getString("ReceiptPrintService.132") + ": " + reportDateFormat.format(ticket.getDeliveryDate())); //$NON-NLS-1$
					endRow(ticketHeaderBuilder);
				}
			}
		}
	...
	private static StringBuilder buildMultiCurrencyTotalAmount(Ticket ticket, TicketPrintProperties printProperties) {
		...
		beginRow(currencyAmountBuilder);
		addColumn(currencyAmountBuilder, "<b>" + Messages.getString("ReceiptPrintService.133") + "</b>");
		endRow(currencyAmountBuilder);
		...
		addColumn(currencyAmountBuilder, getHtmlText(Messages.getString("ReceiptPrintService.134"), 12, CENTER));//$NON-NLS-1$
		//addColumn(currencyAmountBuilder, getHtmlText("Paid", 10, CENTER));//$NON-NLS-1$
		addColumn(currencyAmountBuilder, getHtmlText(Messages.getString("ReceiptPrintService.135"), 12, CENTER));//$NON-NLS-1$
		endRow(currencyAmountBuilder);
	...
	private static StringBuilder buildMultiCurrency(Ticket ticket, TicketPrintProperties printProperties) {
		...
		beginRow(currencyAmountBuilder);
		addColumn(currencyAmountBuilder, groupSettleTickets + "<b>" + Messages.getString("ReceiptPrintService.133") + "</b>");
		endRow(currencyAmountBuilder);
		...
		beginRow(currencyAmountBuilder);
		addColumn(currencyAmountBuilder, getHtmlText("", 10, CENTER));//$NON-NLS-1$
		addColumn(currencyAmountBuilder, getHtmlText(Messages.getString("ReceiptPrintService.136"), 12, CENTER));//$NON-NLS-1$
		addColumn(currencyAmountBuilder, getHtmlText(Messages.getString("ReceiptPrintService.137"), 12, CENTER));//$NON-NLS-1$
		endRow(currencyAmountBuilder);
	...
	public static JasperPrint createKitchenPrint(KitchenTicket ticket) throws Exception {
		...
		map.put(REPORT_DATE, POSConstants.RECEIPT_REPORT_DATE_LABEL + reportDateFormat.format(new Date())); //$NON-NLS-1$
	...
	public static JasperPrint createKitchenPrint(String virtualPrinterName, KitchenTicket ticket, String deviceName) throws Exception {
		...
		map.put("printerName", Messages.getString("ReceiptPrintService.138") + " : " + virtualPrinterName); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
