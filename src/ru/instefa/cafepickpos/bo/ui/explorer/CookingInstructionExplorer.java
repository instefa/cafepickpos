...
import ru.instefa.cafepickpos.model.dao.CookingInstructionDAO;
...
public class CookingInstructionExplorer extends TransparentPanel {
	...
	public CookingInstructionExplorer() {
		...
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					String instruction = JOptionPane.showInputDialog(POSUtil.getBackOfficeWindow(), ru.instefa.cafepickpos.POSConstants.ENTER_INSTRUCTION_DESCRIPTION);
					if (instruction == null) {
						BOMessageDialog.showError(POSUtil.getBackOfficeWindow(), ru.instefa.cafepickpos.POSConstants.INSTRUCTION_CANNOT_BE_EMPTY);
						return;
					}
					if (instruction.length() > 60) {
						BOMessageDialog.showError(POSUtil.getBackOfficeWindow(), ru.instefa.cafepickpos.POSConstants.LONG_INSTRUCTION_ERROR);
						return;
					}
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (ConfirmDeleteDialog.showMessage(CookingInstructionExplorer.this, ru.instefa.cafepickpos.POSConstants.CONFIRM_DELETE,
	...
	class CookingInstructionTableModel extends AbstractTableModel {
		String[] columnNames = { ru.instefa.cafepickpos.POSConstants.ID, ru.instefa.cafepickpos.POSConstants.DESCRIPTION };
