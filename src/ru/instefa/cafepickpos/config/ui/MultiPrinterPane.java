...
import ru.instefa.cafepickpos.model.dao.VirtualPrinterDAO;
...
public class MultiPrinterPane extends JPanel {
	...
	public MultiPrinterPane(String title, List<Printer> allPrinters) {
		...
		JButton btnTest = new JButton(Messages.getString("MultiPrinterPane.3")); //$NON-NLS-1$
		...
		tableModel.addColumn(Messages.getString("MultiPrinterPane.4"), "deviceName"); //$NON-NLS-1$ //$NON-NLS-2$
		tableModel.addColumn(Messages.getString("MultiPrinterPane.5"), "type"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected void doAddPrinter() {
		AddPrinterDialog dialog = new AddPrinterDialog();
		dialog.titlePanel.setTitle(VirtualPrinter.PRINTER_TYPE_NAMES[selectedPrinterType] +
								   " - " + Messages.getString("MultiPrinterPane.4")); //$NON-NLS-1$
		dialog.open();
