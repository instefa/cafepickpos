...
public class DefaultCustomerListView extends CustomerSelector {
	...
	public void initUI() {
		...
		PosButton btnKeyboard = new PosButton(IconFactory.getDefaultIcon("keyboard.png")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		btnNext = new PosButton(POSConstants.BUTTON_TEXT_NEXT);
		...
		btnPrevious = new PosButton(POSConstants.BUTTON_TEXT_PREVIOUS);
		...
		qwertyKeyPad = new ru.instefa.cafepickpos.swing.QwertyKeyPad();
	...
	protected void doSearchCustomer() {
		try {
			...
			lblNumberOfItem.setText(String.format(Messages.getString("DefaultCustomerListView.2"), startNumber, endNumber, totalNumber));
