...
public class PosWindow extends JFrame implements WindowListener {
	...
	private static final SimpleDateFormat dateFormat = new SimpleDateFormat(DateUtil.getDatePattern("hh:mm:ss aaa"));
	...
	public PosWindow() {
		...
		ImageIcon icon = IconFactory.getDefaultIcon("footer_logo.png");
	...
	public void setVisibleWelcomeHeader(boolean visible) {
		...
		JLabel titleLabel = new JLabel(IconFactory.getDefaultIcon("title.png")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void updateView() {
		lblTerminal.setText(POSConstants.TERMINAL_LABEL + ": " + TerminalConfig.getTerminalId());

		User currentUser = Application.getCurrentUser();
		if (currentUser != null) {
			lblUser.setText(POSConstants.USER + ": " +
					currentUser.getFullName() + " (" +
					currentUser.getType().getName() + ")");
		}
		else {
			lblUser.setText(Messages.getString("PosWindow.1")); //$NON-NLS-1$
		}

		lblDB.setText(Messages.getString("PosWindow.0") + ": " +
				AppConfig.getDatabaseHost() +
				"/" + AppConfig.getDatabaseName());
		if(Application.getInstance().isPriceIncludesTax()) {
			lblTaxInculed.setText(Messages.getString("PosWindow.2"));
		}else {
			lblTaxInculed.setText(Messages.getString("PosWindow.3"));
		}
	}
	...
	public void setupSizeAndLocation() {
		...
		setSize(AppConfig.getInt(WWIDTH, (int) screenSize.getWidth()), AppConfig.getInt(WHEIGHT, (int) screenSize.getHeight() - border));

		setLocation(AppConfig.getInt(WLOCX, ((screenSize.width - getWidth()) >> 1)), AppConfig.getInt(WLOCY, ((screenSize.height - getHeight()) >> 1)));
		setMinimumSize(new Dimension(1024, 768 - border));
	...
	private class ClockTimerHandler implements ActionListener {
		...
		private void showFooterTimer() {
			...
			lblTime.setText(POSConstants.TIME_CAPITALIZED + ": " + sb.toString());
		}
	}
}