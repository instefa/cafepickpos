...
import ru.instefa.cafepickpos.model.util.TicketSummary;
...
public class TicketDAO extends BaseTicketDAO {
	...
	public List<Ticket> findNextCustomerTickets(Integer customerId, PaginatedTableModel tableModel, String filter) {
		...
		try {
			...
			if (filter.equals(PaymentStatusFilter.OPEN.getName())) {
				criteria.add(Restrictions.eq(Ticket.PROP_PAID, Boolean.FALSE));
				criteria.add(Restrictions.eq(Ticket.PROP_CLOSED, Boolean.FALSE));
			}
	...
	//	public void saveTransaction(Ticket ticket, ru.instefa.cafepickpos.model.PosTransaction transaction, Terminal terminal, User user) throws Exception {
	//		Session session = null;
	//    	Transaction tx = null;
	//    	try {
	...
	private void updateCriteriaFilters(Criteria criteria) {
		User user = Application.getCurrentUser();
		PaymentStatusFilter paymentStatusFilter = ru.instefa.cafepickpos.config.TerminalConfig.getPaymentStatusFilter();
		String orderTypeFilter = ru.instefa.cafepickpos.config.TerminalConfig.getOrderTypeFilter();
