 ...
 * ************************************************************************
 */
package ru.instefa.cafepickpos.ui.forms;
...
import ru.instefa.cafepickpos.model.util.ZipCodeUtil;
...
public class QuickCustomerForm extends BeanEditor<Customer> {
	...
	private void createCustomerForm() {
        ...
        // 20170726, pymancer, POSConstants.NAME has too general translation
		//JLabel lblName = new JLabel(POSConstants.NAME); //$NON-NLS-1$
        JLabel lblName = new JLabel(Messages.getString("CustomerForm.3"));
	...
	public boolean delete() {
		try {
			...
			int option = POSMessageDialog.showYesNoQuestionDialog(POSUtil.getBackOfficeWindow(),
						 Messages.getString("Are you sure to delete selected table?"),
		 				 POSConstants.CONFIRM); //$NON-NLS-1$ //$NON-NLS-2$
