...
import ru.instefa.cafepickpos.ui.dialog.TransactionCompletionDialog;
import ru.instefa.cafepickpos.ui.views.TicketDetailView;
...
import ru.instefa.cafepickpos.util.GlobalIdGenerator;
...
public class GroupSettleTicketDialog extends POSDialog implements CardInputListener {
	...
	private JPanel createTicketInfoPanel() {
		...
		lblCustomer.setText("ReceiptPrintService.0"); //$NON-NLS-1$

		labelCustomer = new JLabel();

		JPanel ticketInfoPanel = new ru.instefa.cafepickpos.swing.TransparentPanel(new MigLayout("wrap 2,fill, hidemode 3", "[][grow]", "")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	private JPanel createTotalViewerPanel() {
		...
		lblDeliveryCharge.setText(Messages.getString("SettleTicketDialog.24") + " " +
		...
		lblTax.setText(ru.instefa.cafepickpos.POSConstants.TAX + ":" + " " +
            CurrencyUtil.getCurrencySymbol()); //$NON-NLS-1$ //$NON-NLS-2$
		...
		lblTotal.setText(ru.instefa.cafepickpos.POSConstants.TOTAL + ":" + " " +
            CurrencyUtil.getCurrencySymbol()); //$NON-NLS-1$ //$NON-NLS-2$
		...
		JPanel ticketAmountPanel = new ru.instefa.cafepickpos.swing.TransparentPanel(
            new MigLayout("hidemode 3,ins 2 2 3 2,alignx trailing,fill", "[grow][]", "")
	...
	public void doGroupSettle(PaymentType paymentType) {
		try {
			...
			if (totalTenderAmount < totalDueAmount) {
				POSMessageDialog.showMessage("GroupSettleTicketDialog.0"); //$NON-NLS-1$
				return;
			}
	...
	public static void printTicket(Ticket ticket, PosTransaction transaction) {
		try {
			...
			POSMessageDialog.showError(Application.getPosWindow(), ru.instefa.cafepickpos.POSConstants.PRINT_ERROR, ee);
		}
	}
