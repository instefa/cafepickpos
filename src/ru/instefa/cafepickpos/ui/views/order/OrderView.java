...
import ru.instefa.cafepickpos.model.TicketItemCookingInstruction;
...
import ru.instefa.cafepickpos.ui.dialog.MiscTicketItemDialog;
...
import ru.instefa.cafepickpos.ui.dialog.SeatSelectionDialog;
...
import ru.instefa.cafepickpos.ui.views.CookingInstructionSelectionView;
import ru.instefa.cafepickpos.ui.views.order.actions.TicketEditListener;
import ru.instefa.cafepickpos.ui.views.payment.PaymentListener;
import ru.instefa.cafepickpos.ui.views.payment.PaymentView;
...
public class OrderView extends ViewPanel implements PaymentListener, TicketEditListener {
	...
	private PosButton btnDeliveryInfo = new PosButton(Messages.getString("OrderView.2"));
	...
	private void initComponents() {
		...
		//		addView("VIEW_EMPTY", new ru.instefa.cafepickpos.swing.TransparentPanel()); //$NON-NLS-1$
		...
		btnCancel.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				int result = POSMessageDialog.showYesNoQuestionDialog(null,
						Messages.getString("OrderView.3"),
		...
		btnSend.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					POSMessageDialog.showMessage(Messages.getString("OrderView.4"));
				} catch (StaleStateException x) {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					String password = PasswordEntryDialog.show(Application.getPosWindow(),
															   Messages.getString("PosAction.0"));
						...
						POSMessageDialog.showError(Application.getPosWindow(),
												   Messages.getString("PosAction.1"));
							return;
						}
						else {
					...
					if (!currentTicket.isBarTab() && (ticketView.getTicket().getTicketItems() == null ||
                        ticketView.getTicket().getTicketItems().size() == 0)) {
                            POSMessageDialog.showError(ru.instefa.cafepickpos.POSConstants.TICKET_IS_EMPTY_
                        );
						return;
					}
	...
	private JPanel createTicketSummeryPanel() {
		...
		lblDeliveryCharge.setText(Messages.getString("OrderView.8") + " " +
            CurrencyUtil.getCurrencySymbol()); //$NON-NLS-1$ //$NON-NLS-2$
		...
		lblGratuity.setText(Messages.getString("SettleTicketDialog.5") + ":" + " " +
            CurrencyUtil.getCurrencySymbol()); //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
		...
		JPanel ticketAmountPanel = new ru.instefa.cafepickpos.swing.TransparentPanel(
            new MigLayout("hidemode 3,ins 2 2 3 2,alignx trailing,fill", "[grow]2[]", "")
	...
	protected void btnCustomerNumberActionPerformed() {// GEN-FIRST:event_btnCustomerNumberActionPerformed
		...
		dialog.setTitle(ru.instefa.cafepickpos.POSConstants.NUMBER_OF_GUESTS);
		...
		if (guestNumber == 0) {
			POSMessageDialog.showError(Application.getPosWindow(), ru.instefa.cafepickpos.POSConstants.GUEST_NUMBER_CANNOT_BE_0);
			return;
		}
	...
	protected void doAddSeatNumber() {// GEN-FIRST:event_btnCustomerNumberActionPerformed
		...
		seatDialog.setTitle(Messages.getString("TicketView.47"));
		seatDialog.pack();
		seatDialog.open();
		...
		if (seatNumber == -1) {
			NumberSelectionDialog2 dialog = new NumberSelectionDialog2();
			dialog.setTitle(Messages.getString("TicketView.48"));
			dialog.pack();
			dialog.open();
	...
	private void doAddSeatTreatTicketItem(Integer seatNumber) {
		...
		if (seatNumber == 0)
			ticketItem.setName(Messages.getString("OrderView.5"));
		else
			ticketItem.setName(Messages.getString("TicketView.49") + " " + seatNumber);
		...
		if (currentTicket != null) {
			...
			if (!type.isShowTableSelection()) {
				...
				if (tableNumbers != null) {
					...
					btnTableNumber.setText("<html><center>" + Messages.getString("OrderView.6") +
										   ": " + tables + "</center><html/>");

				}
				else {
					btnTableNumber.setText(Messages.getString("OrderView.6"));
				}
	...
	public ru.instefa.cafepickpos.ui.views.order.CategoryView getCategoryView() {
		return categoryView;
	}

	public void setCategoryView(ru.instefa.cafepickpos.ui.views.order.CategoryView categoryView) {
	...
	public ru.instefa.cafepickpos.ui.views.order.TicketView getTicketView() {
		return ticketView;
	}

	public void setTicketView(ru.instefa.cafepickpos.ui.views.order.TicketView ticketView) {
	...
	public void setVisible(boolean aFlag) {
		if (aFlag) {
			try {
				...
				POSMessageDialog.showError(Application.getPosWindow(), ru.instefa.cafepickpos.POSConstants.ERROR_MESSAGE, t);
			}
		}
		else {
