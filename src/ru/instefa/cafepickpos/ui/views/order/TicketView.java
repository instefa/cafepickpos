...
import ru.instefa.cafepickpos.ui.dialog.ItemSearchDialog;
...
import ru.instefa.cafepickpos.ui.views.CashierSwitchBoardView;
...
public class TicketView extends JPanel {
	...
	private ru.instefa.cafepickpos.swing.PosButton btnDelete = new PosButton();
	private ru.instefa.cafepickpos.swing.PosButton btnIncreaseAmount = new PosButton();
	private ru.instefa.cafepickpos.swing.PosButton btnEdit = new PosButton("..."); //$NON-NLS-1$ //$NON-NLS-2$
	private ru.instefa.cafepickpos.swing.PosButton btnScrollDown;
	private ru.instefa.cafepickpos.swing.PosButton btnScrollUp = new PosButton();
	...
	private ru.instefa.cafepickpos.ui.ticket.TicketViewerTable ticketViewerTable;
	...
	private void initComponents() {
		...
		ticketViewerTable = new ru.instefa.cafepickpos.ui.ticket.TicketViewerTable();
	...
	private void createItemSearchPanel() {
		...
		btnSearch.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				dialog.setTitle(POSConstants.SEARCH_ITEM_BUTTON_TEXT);
				dialog.pack();
				dialog.open();
				...
				if (!addMenuItemByBarcode(dialog.getValue())) {
					if (!addMenuItemByItemId(dialog.getValue())) {
						POSMessageDialog.showError(Application.getPosWindow(), Messages.getString("TicketView.45"));
					}
				}
			}
		});
		itemSearchPanel.add(txtSearchItem, "split 2, grow,span");
		itemSearchPanel.add(btnSearch, "grow, span, width " + PosUIManager.getSize(60) +
							"!, height " + PosUIManager.getSize(40) + "!");
	}
	...
	private boolean filterByStockAmount(MenuItem menuItem) {
		if (menuItem.isDisableWhenStockAmountIsZero() && menuItem.getStockAmount() <= 0) {
			POSMessageDialog.showError(Messages.getString("TicketView.46"));
			return false;
		}
		return true;
	}
	...
	private void createTicketItemControlPanel() {
		...
		btnScrollUp.setIcon(IconFactory.getIcon("up.png", size)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		btnIncreaseAmount.setIcon(IconFactory.getIcon("add_user.png", size)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		btnDecreaseAmount.setIcon(IconFactory.getIcon("minus.png", size)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		btnScrollDown.setIcon(IconFactory.getIcon("down.png", size)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		btnDelete.setIcon(IconFactory.getIcon("delete.png", size)); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public synchronized void updateModel() {
		if (!ticket.isBarTab() && (ticket.getTicketItems() == null || ticket.getTicketItems().size() == 0)) {
			throw new PosException(ru.instefa.cafepickpos.POSConstants.TICKET_IS_EMPTY_);
		}
	...
	public void updateView() {
		if (ticket == null) {
			...
			titledBorder.setTitle(ticket.getTicketType().toString() + Messages.getString("TicketView.50")); //$NON-NLS-1$
			return;
		}
		ticket.calculatePrice();
		...
		if (ticket.getId() == null) {
			titledBorder.setTitle(ticket.getTicketType() + " " + Messages.getString("TicketView.50")); //$NON-NLS-1$
		}
		else {
			titledBorder.setTitle(ticket.getTicketType() +
								  " " + Messages.getString("TicketView.37") + ticket.getId() + " " +
								  Messages.getString("TicketView.51") + " " +
								  getTableNumbers(ticket.getTableNumbers())); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public ru.instefa.cafepickpos.ui.ticket.TicketViewerTable getTicketViewerTable() {
	...
	private String getDisplayMessage(ITicketItem item, String totalPrice) {
		...
		String total = POSConstants.TOTAL + CurrencyUtil.getCurrencySymbol();
	...
	private double getNewItemQuantity() {
		...
		if (TerminalConfig.getScaleActivationValue().equals("cas10")) {
			...
			selectedQuantity = BasicWeightInputDialog.takeDoubleInput(Messages.getString("TicketView.52"), 1);
		}
		if (selectedQuantity <= -1) {
			return -1;
		}
		...
		if (selectedQuantity == 0) {
			POSMessageDialog.showError(Messages.getString("TicketView.53"));
			return -1;
		}
