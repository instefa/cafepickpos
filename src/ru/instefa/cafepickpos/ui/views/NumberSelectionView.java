...
public class NumberSelectionView extends TransparentPanel implements ActionListener {
	...
	private void initComponents() {
		...
		btnClearAll.setText(ru.instefa.cafepickpos.POSConstants.CLEAR_ALL);
		btnClearAll.setActionCommand(ru.instefa.cafepickpos.POSConstants.CLEAR_ALL);
		...
		String[][] numbers = { { "7", "8", "9" }, //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				...
				{ ".", "0", ru.instefa.cafepickpos.POSConstants.CLEAR_ALL} //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		};
		...
		for (int i = 0; i < numbers.length; i++) {
			for (int j = 0; j < numbers[i].length; j++) {
				...
				ImageIcon icon = IconFactory.getIcon(iconNames[i][j]); //$NON-NLS-1$
				...
				if (icon == null) {
					...
					if (ru.instefa.cafepickpos.POSConstants.CLEAR_ALL.equals(buttonText)) {
						posButton.setText(buttonText);
					}
				}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (actionCommand.equals(ru.instefa.cafepickpos.POSConstants.CLEAR_ALL)) {
			tfNumber.setText("0"); //$NON-NLS-1$
		}
		else if (actionCommand.equals(ru.instefa.cafepickpos.POSConstants.CLEAR)) {
