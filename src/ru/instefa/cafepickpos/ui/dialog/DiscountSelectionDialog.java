...
public class DiscountSelectionDialog extends OkCancelOptionDialog implements ActionListener {
	...
	private void createCouponSearchPanel() {
		...
		JLabel lblCoupon = new JLabel(Messages.getString("DiscountSelectionDialog.3"));

		txtSearchItem = new JTextField();
		txtSearchItem.addFocusListener(new FocusListener() {
			@Override
			public void focusLost(FocusEvent e) {
				txtSearchItem.setText(Messages.getString("DiscountSelectionDialog.4"));
		...
		txtSearchItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (txtSearchItem.getText().equals("")) {
					POSMessageDialog.showMessage(Messages.getString("DiscountSelectionDialog.5"));
					return;
				}
				...
				if (!addCouponByBarcode(dialog.getValue())) {
					if (!addCouponById(dialog.getValue())) {
						POSMessageDialog.showError(Application.getPosWindow(),
                            Messages.getString("DiscountSelectionDialog.7"));
					}
				}
			}
		});
	...
	private double getModifiedValue(Discount discount) {
		Double newValue = NumberSelectionDialog2.takeDoubleInput(Messages.getString("DiscountSelectionDialog.8"),
															     Messages.getString("DiscountSelectionDialog.8"),
																 discount.getValue()); //$NON-NLS-1$ //$NON-NLS-2$
		if (newValue > 0) {
			return newValue;
		}
		return 0;
