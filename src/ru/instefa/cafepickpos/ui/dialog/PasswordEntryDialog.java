	...
	private JPanel createKeyboardPanel() {
		...
		for (int i = 0; i < numbers.length; i++) {
			for (int j = 0; j < numbers[i].length; j++) {
				...
				ImageIcon icon = ru.instefa.cafepickpos.IconFactory.getIcon(iconNames[i][j]); //$NON-NLS-1$
				if (icon != null) {
		...
		ImageIcon clearIcon = ru.instefa.cafepickpos.IconFactory.getIcon("clear.png"); //$NON-NLS-1$
		btnClear.setIcon(clearIcon);
		btnClear.setIconTextGap(0);

		ImageIcon clearAllIcon = ru.instefa.cafepickpos.IconFactory.getIcon("clear.png"); //$NON-NLS-1$
	...
	private synchronized boolean checkLogin(String secretKey) {
		...
		if (LoginView.getInstance().isBackOfficeLogin()) {
			if (!user.hasPermission(UserPermission.VIEW_BACK_OFFICE)) {
				statusLabel.setText(Messages.getString("PasswordEntryDialog.31"));
				return false;
			}
			return true;
		}
		...
		if (isAutoLogOffMode()) {
            ...
            if (TerminalConfig.getDefaultView().equals(BookingView.VIEW_NAME)) {
				if (!user.hasPermission(UserPermission.TABLE_BOOKING)) {
					statusLabel.setText(Messages.getString("PasswordEntryDialog.31"));
					return false;
				}
			}
