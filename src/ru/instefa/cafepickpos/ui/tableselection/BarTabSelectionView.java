...
import ru.instefa.cafepickpos.swing.BarTabButton;
...
public class BarTabSelectionView extends JPanel {
	...
	private void init() {
		...
		TitledBorder titledBorder1 = BorderFactory.createTitledBorder(null,
            Messages.getString("BarTabSelectionView.1"),
	...
	private void rendererBarTickets() {
		...
		for (Ticket ticket : openTickets) {
			...
			if (customerName == null) {
				customerName = Messages.getString("BarTabSelectionView.0");
			}
			barTabButton
					.setText("<html><center>" + customerName + "<br><h4 style='margin:0px;'>"
                        + ticket.getOwner().getFirstName()
                        + "<br>" + Messages.getString("TableSelectionView.4")
							+ ticket.getId()
                        + "</h4>" + CurrencyUtil.getCurrencySymbol() + ticket.getTotalAmount()
                        + "<br><small style='margin:0px;'>Due: " + CurrencyUtil.getCurrencySymbol()
                        + ticket.getDueAmount() + "</small></center></html>"); //$NON-NLS-1$
